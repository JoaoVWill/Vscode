...
echo "Agora gerar o IP da aplicação para acessar o site default do NGINX de maneira externa"
...
sudo bash -c "echo 'server {' > reverse-proxy.conf"
sudo bash -c "echo '    listen 80;' >> reverse-proxy.conf"
sudo bash -c "echo '    location / {' >> reverse-proxy.conf"
sudo bash -c "echo '        proxy_pass http://'$NODE_IP':'$NODE_PORT';' >> reverse-proxy.conf"
sudo bash -c "echo '    }' >> reverse-proxy.conf"
sudo bash -c "echo '}' >> reverse-proxy.conf"
